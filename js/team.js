...
(() => {
 ...
 let data=null,key='',admin=false,squad='main',tab='events',eventId=new URLSearchParams(location.search).get('event')||'',past=false,selected='',actor=get('actor')||'管理者',busy=false,edit=null,memberNumberDesc=false;
 let year=M.yearFor(M.today(),4),calendarMonth=M.today().slice(0,7),timer;
 const roleName={player:'選手',staff:'スタッフ',ac:'AC',guest:'体験・ゲスト'};
 const actionName={grant_admin:'管理者権限を付与',answer:'出欠を更新',member:'名簿を更新',archive_member:'メンバーの利用状態を変更',delete_member:'メンバーを完全削除',event:'予定を保存',delete_event:'予定を削除',guest:'助っ人を更新',notice:'伝達事項を更新',ledger:'会計を保存',delete_ledger:'会計を削除',bib:'ビブスを更新',settings:'設定を更新',rotate_link:'共有リンクを変更',undo_answer:'出欠を取り消し'};
 ...
 const numberCollator=new Intl.Collator('ja',{numeric:true,sensitivity:'base'});
 const byMemberNumber=(a,b)=>{const an=String(a.number||'').trim(),bn=String(b.number||'').trim();if(!an&&!bn)return numberCollator.compare(a.name,b.name);if(!an)return 1;if(!bn)return -1;return numberCollator.compare(an,bn)||numberCollator.compare(a.name,b.name);};
 const members=()=>data.members.filter(m=>m.squad===squad&&m.active).sort(byMemberNumber);
 ...
 function render(){
  ...
  const nav=[['events','出欠'],['members','名簿'],['annual','参加回数'],...(admin?[['ledger','会計']]:[]),['history','変更履歴'],...(admin?[['settings','設定']]:[])];
  ...
  ${tab==='events'?eventsView(sel):tab==='members'?membersView():tab==='annual'?annualView():tab==='ledger'?ledgerView():tab==='bibs'?bibsView():tab==='settings'?settingsView():historyView()}`;
 }
 function parking(e){const count=M.count(data,e).cars;return e.asks_car?'<strong class="parking-count '+(count>=16?'parking-over':'')+'">'+count+'台'+(count>=16?'（16台以上）':'')+'</strong>':'—';}
 function calendarView(){
  const [y,m]=calendarMonth.split('-').map(Number),offset=new Date(Date.UTC(y,m-1,1)).getUTCDay(),days=new Date(Date.UTC(y,m,0)).getUTCDate();
  const events=data.events.filter(e=>e.squad===squad&&e.event_date.startsWith(calendarMonth)).sort((a,b)=>(a.start_time||'').localeCompare(b.start_time||''));
  let cells='<div class="calendar-day blank" aria-hidden="true"></div>'.repeat(offset);
  for(let day=1;day<=days;day++){const date=calendarMonth+'-'+String(day).padStart(2,'0');cells+='<div class="calendar-day '+(date===M.today()?'today':'')+'"><span class="calendar-number">'+day+'</span>'+events.filter(e=>e.event_date===date).map(e=>'<button class="calendar-event" data-action="open-event" '+attr('id',e.id)+' aria-label="'+esc(dateLabel(date)+' '+e.place+'の出欠を開く')+'"><strong>'+esc(e.start_time?.slice(0,5)||'時間未定')+'</strong><span>'+esc(e.place||'場所未定')+'</span><span>駐車希望 '+parking(e)+'</span></button>').join('')+'</div>';}
  cells+='<div class="calendar-day blank" aria-hidden="true"></div>'.repeat((7-(offset+days)%7)%7);
  return '<section class="panel calendar-panel"><div class="section-head"><h2>'+y+'年'+m+'月</h2><div class="actions">'+btn('前月','calendar-prev')+btn('今月','calendar-today')+btn('翌月','calendar-next')+'</div></div><p class="hint">カレンダー内の予定、または下のイベントカードから出欠ページを開けます。駐車希望は出席者と助っ人の合計です。</p><div class="calendar-grid">'+['日','月','火','水','木','金','土'].map(d=>'<div class="calendar-weekday">'+d+'</div>').join('')+cells+'</div></section>';
 }
 function eventsView(sel){
  ...
  return calendarView()+'<div class="section-head"><div><h2>イベント一覧</h2><p class="hint">カードの見出しから参加状況を確認できます。自分の回答は「出欠を回答する」から。</p></div>'+(admin?btn('＋ 予定を作る','event-new','',''):'')+'</div><div class="toolbar">'+btn(past?'これからの予定':'過去の予定','past')+(admin?btn('＋ メンバー追加','member-new'):'')+'</div><div class="event-card-list">'+list.map(e=>{const a=sel?answer(e.id,sel.id):null,c=M.count(data,e);return '<article class="panel home-event"><button class="event-card-heading" data-action="open-event" '+attr('id',e.id)+' aria-label="'+esc(dateLabel(e.event_date)+' '+(e.place||'場所未定')+'の出欠を開く')+'"><span class="home-date"><span>'+Number(e.event_date.slice(5,7))+'月</span><strong>'+Number(e.event_date.slice(8,10))+'</strong></span><span class="home-event-title"><span class="tag">'+(e.kind==='match'?'試合':'練習')+'</span><strong>'+esc(e.place||'場所未定')+'</strong><span class="muted">'+esc(dateLabel(e.event_date))+' · '+esc(e.start_time?.slice(0,5)||'時間未定')+(e.end_time?' – '+esc(e.end_time.slice(0,5)):'')+'</span></span><span aria-hidden="true">›</span></button><div class="home-event-counts"><span>出席 <strong>'+c.total+'人</strong></span><span>駐車希望 '+parking(e)+'</span></div>'+(sel?'<p><strong>'+esc(sel.name)+'さんの出欠：</strong>'+(a?.status==='yes'?'○ 出席':a?.status==='no'?'× 欠席':'未回答')+'</p>'+btn('出欠を回答する','answer',attr('ev',e.id)+' '+attr('mid',sel.id),'home-answer'):'')+(admin?'<div class="actions" style="margin-top:12px">'+btn('共有','share-event',attr('id',e.id))+'</div>':'')+'</article>';}).join('')+'</div>'+(list.length?'':empty('予定がまだありません。'));
 }
 ...
 function syncPlate(){const f=document.getElementById('edit-form'),plate=f.elements.vehicle_plate;if(!plate)return;const car=f.elements.car;const mode=car?.type==='checkbox'?(car.checked?'yes':f.elements.uses_bicycle?.checked?'bicycle':'no'):car?.value;const needs=f.elements.status?.value==='yes'&&(mode==='yes'||mode==='bicycle');plate.required=needs&&!edit?.hasPlate;plate.disabled=!needs;plate.closest('label').hidden=!needs;}
 function eventView(e){const own=data.me?answer(e.id,data.me.id):null,c=M.count(data,e);const gs=data.guests.filter(g=>g.event_id===e.id);return `<div class="section-head"><div><h2>${esc(dateLabel(e.event_date))} ${esc(e.place)}</h2><div class="muted">メンバー ${members().length}人 · 助っ人 ${gs.filter(g=>g.status==='yes').length}人出席</div></div><div class="actions">${admin?btn('共有','share-event',attr('id',e.id)):''}${admin?btn('Excelを出力','export-event',attr('id',e.id),''):''}${btn('イベント一覧へ戻る','event-back')}</div></div>
  <section class="panel">${data.me?'<p>自分の出欠：<strong>'+(own?.status==='yes'?'○ 出席':own?.status==='no'?'× 欠席':'未回答')+'</strong></p>'+btn('自分の出欠を登録・変更','answer',attr('ev',e.id)+' '+attr('mid',data.me.id),''):''}<p><strong>時間：</strong>${esc(e.start_time?.slice(0,5)||'時間未定')}${e.end_time?' – '+esc(e.end_time.slice(0,5)):''}</p><p>${e.deadline?'回答期限：'+esc(dateLabel(e.deadline)):''} ${e.booking_person?'予約担当：'+esc(e.booking_person):''}</p><p class="note">${esc(e.note)}</p><div class="metrics"><div><b>${c.total}</b>出席（助っ人${c.guests}）</div><div><b>${c.no}</b>欠席</div><div><b>${c.unknown}</b>未回答</div><div><b class="${c.cars>=16?'parking-over':''}">${e.asks_car?c.cars:'—'}</b>車${e.asks_car&&c.cars>=16?'（16台以上）':''}</div></div>${admin?'<p class="hint">各参加者の「回答・修正」から代理で出欠を変更できます。</p><div class="actions">'+btn('予定を編集','event-edit',attr('id',e.id))+btn('予定を複製','event-copy',attr('id',e.id))+'</div>':''}</section>
  <div class="table-wrap"><table><thead><tr><th>番号</th><th>名前</th><th>区分</th><th>出欠</th><th>車</th><th>備考</th><th>操作</th></tr></thead><tbody>${members().map(m=>{const a=answer(e.id,m.id);return `<tr><td>${esc(m.number)}</td><td>${esc(m.name)}</td><td>${roleName[m.member_role]}</td><td class="${a?.status||'unknown'}">${a?.status==='yes'?'○ 出席':a?.status==='no'?'× 欠席':'未回答'}</td><td>${a?.uses_bicycle?'自転車':a?.car==='yes'?'利用する':a?.car==='no'?'利用しない':'—'}${admin&&a?.vehicle_plate?`<div class="hint">${esc(a.vehicle_plate)}</div>`:''}</td><td class="note">${esc(a?.note)}</td><td>${admin||squad==='junior'||data.me?.id===m.id?btn('回答・修正','answer',`${attr('ev',e.id)} ${attr('mid',m.id)}`):'—'}</td></tr>`;}).join('')}</tbody></table></div>
 ...
 function membersView(){const list=data.members.filter(m=>m.squad===squad&&(m.active||admin)).slice().sort((a,b)=>memberNumberDesc?-byMemberNumber(a,b):byMemberNumber(a,b));return `<div class="section-head"><div><h2>メンバー名簿</h2><div class="hint">${squad==='junior'?'名前と区分だけで追加できます。':'Googleの登録メールと自動で紐づきます。名簿の追加・編集は管理者が行います。'} 番号の見出しで昇順・降順を切り替えられます。</div></div>${admin||squad==='junior'?btn('＋ 追加','member-new','',''):''}</div><div class="table-wrap"><table><thead><tr><th><button type="button" class="table-sort" data-action="member-number-sort">番号 ${memberNumberDesc?'▼':'▲'}</button></th><th>名前</th><th>区分</th><th>学年</th><th>所属・Jr.情報</th><th>代理入力者</th><th>背番号</th><th></th></tr></thead><tbody>${list.map(m=>`<tr><td>${esc(m.number)}</td><td>${esc(m.name)}${m.active?'':'（削除済み）'}</td><td>${roleName[m.member_role]}</td><td>${esc(m.grade)}</td><td class="note">${esc(m.affiliation)}</td><td>${esc(m.proxy_name)}</td><td>${esc(m.jersey)}</td><td>${m.active&&(admin||squad==='junior')?btn('編集','member-edit',attr('id',m.id)):''}${admin&&m.squad==='main'?(m.is_admin?'<span class="tag">管理者</span>':m.active&&m.email?btn('管理者権限を付与','grant-admin',attr('id',m.id)):''):''}${admin?btn(m.active?'メンバーを削除':'元に戻す','archive',`${attr('id',m.id)} data-active="${!m.active}"`,m.active?'danger small':'secondary small'):''}${admin&&!m.active?btn('完全に削除','delete-member',attr('id',m.id),'danger small'):''}</td></tr>`).join('')}</tbody></table></div>${members().length?'':empty('最初のメンバーを追加しましょう。')}`;}
 function annualView(){const [start,end]=M.yearRange(year,data.year_start_month);return `<div class="section-head"><h2>年度別の参加回数</h2><div class="toolbar"><label for="year">年度</label><input class="timeline-year" id="year" type="number" min="2000" max="2100" value="${year}"></div></div><p class="hint">${start} 〜 ${Number(end.slice(0,4))}年${data.year_start_month}月1日の前日。今日までの「出席」回答を集計（未来の予定は除く）。過去の記録を登録すると反映されます。起算月は管理者が変更できます。</p><div class="table-wrap"><table><thead><tr><th>番号</th><th>名前</th><th>区分</th><th class="num">参加回数</th></tr></thead><tbody>${data.members.filter(m=>m.squad===squad).sort(byMemberNumber).map(m=>`<tr><td>${esc(m.number)}</td><td>${esc(m.name)}${m.active?'':'（休会）'}</td><td>${roleName[m.member_role]}</td><td class="num">${M.annual(data,m,year)}回</td></tr>`).join('')}</tbody></table></div>`;}
 ...
 function eventPresets(v){
  const events=data.events.filter(e=>e.squad===squad), ranked=values=>{const counts=new Map();for(const value of values.filter(Boolean))counts.set(value,(counts.get(value)||0)+1);return [...counts].sort((a,b)=>b[1]-a[1]||a[0].localeCompare(b[0],'ja')).map(([value])=>value);};
  const places=ranked(events.map(e=>e.place?.trim())),slots=ranked(events.filter(e=>e.start_time&&e.end_time).map(e=>e.start_time.slice(0,5)+'|'+e.end_time.slice(0,5)));
  return select('place_preset','登録済みの場所から選ぶ',places.includes(v.place)?v.place:'',[['','新しい場所・直接入力'],...places.map(x=>[x,x])])+select('time_preset','登録済みの時間帯から選ぶ',[v.start_time?.slice(0,5),v.end_time?.slice(0,5)].join('|'),[['','新しい時間帯・直接入力'],...slots.map(x=>[x,x.replace('|',' ～ ')])])+'<p class="hint wide">登録回数の多い順です。選択すると下の入力欄に反映されます。直接入力した場所・時間帯も保存後に候補へ加わります。</p>';
 }
 function openShare(e){
  edit=null;const share=M.shareEvent(e,data.team,location.origin,location.pathname);
  document.querySelector('#edit-form button[type="submit"]').hidden=true;
  fields.innerHTML='<h2>イベントを共有</h2><p class="hint wide">送信先はLINE・メール側で選べます。チーム内に共有してください。出欠・車のナンバー・管理者用の鍵は共有しません。</p><textarea class="wide" aria-label="共有する内容" readonly>'+esc(share.body)+'</textarea><div class="actions wide"><a class="button" target="_blank" rel="noopener noreferrer" href="https://line.me/R/share?text='+encodeURIComponent(share.body)+'">LINEで共有</a><a class="button secondary" href="mailto:?subject='+encodeURIComponent(share.title)+'&body='+encodeURIComponent(share.body)+'">メールで共有</a>'+btn('リンクをコピー','copy-event',attr('id',e.id))+'</div>';
  document.getElementById('form-error').textContent='';dialog.showModal();
 }
 function openForm(type,v={},extra={}){
  document.querySelector('#edit-form button[type="submit"]').hidden=false;
  ...
  if(type==='event'){title=extra.copy?'予定を複製':v.id?'予定を編集':'予定を作る';if(extra.copy)edit.id=null;html=(extra.copy?'<p class="wide template-note">日程・締切を入れ直してください。出欠と助っ人は複製しません。</p>':'')+field('event_date','日程',extra.copy?'':v.event_date||M.today(),'date',{required:true})+select('kind','種類',v.kind||'practice',[['practice','練習'],['match','試合']])+eventPresets(v)+field('start_time','開始時刻',v.start_time?.slice(0,5),'time')+field('end_time','終了時刻',v.end_time?.slice(0,5),'time')+field('place','場所',v.place)+field('deadline','回答期限',extra.copy?'':v.deadline,'date')+field('booking_person','予約担当',v.booking_person)+check('asks_car','車の回答を集める',v.asks_car??squad==='main')+area('note','注意事項',v.note)+(!v.id?check('share_after_save','保存後にLINE・メール共有画面を開く',false):'');}
  if(type==='answer'){title=memberName(extra.mid)+'さんの回答';const e=data.events.find(e=>e.id===extra.ev);html=select('status','出欠',v.status||'',[['','未回答'],['yes','○ 出席'],['no','× 欠席']])+(e.asks_car?select('car','車の利用予定は？',v.uses_bicycle?'bicycle':v.car||'',[['','未回答'],['yes','利用する'],['no','利用しない'],['bicycle','バイク']])+field('vehicle_plate','車・バイクのナンバー全体（例：横浜 300 あ 1234）',v.vehicle_plate||((extra.mid===data.me?.id?data.me:data.members.find(m=>m.id===extra.mid))?.vehicle_plate)||'','text',{maxlength:40,wide:true}):'')+area('note','遅刻など、この日の伝達事項',v.note)+'<p class="hint wide">「未回答」は回答とこの日の伝達を取り消します。</p>';}
  if(type==='guest'){title=v.id?'助っ人を修正':'この日の助っ人を追加';html=field('name','助っ人の名前',v.name,'text',{required:true,maxlength:80})+(data.me?`<p>声掛け担当：${esc(data.me.name)}</p>`:field('invited_by','声掛け担当',v.invited_by||actor))+select('status','出欠',v.status||'yes',[['yes','○ 出席'],['no','× 欠席']])+(data.events.find(e=>e.id===(extra.ev||v.event_id))?.asks_car?check('car','車の利用予定は？ 利用する',v.car)+check('uses_bicycle','バイクを利用する',v.uses_bicycle)+field('vehicle_plate','車・バイクのナンバー全体（例：横浜 300 あ 1234）',v.vehicle_plate,'text',{maxlength:40,wide:true}):'')+area('note','伝達事項',v.note);edit.ev=extra.ev||v.event_id;}
 ...
 document.addEventListener('change',e=>{
  if(edit?.type==='event'&&e.target.closest('#edit-form')){const form=document.getElementById('edit-form');
   if(e.target.name==='place_preset'){form.elements.place.value=e.target.value;}
   if(e.target.name==='time_preset'){const [start='',end='']=e.target.value.split('|');form.elements.start_time.value=start;form.elements.end_time.value=end;}
   if(e.target.name==='place')form.elements.place_preset.value='';
   if(['start_time','end_time'].includes(e.target.name))form.elements.time_preset.value='';
  }
 ...
 document.addEventListener('submit',async e=>{
  ...
  if(edit.id)p.id=edit.id;p.squad=squad;delete p.place_preset;delete p.time_preset;const shareAfterSave=p.share_after_save===true;delete p.share_after_save;const beforeEventIds=edit.type==='event'?new Set(data.events.map(x=>x.id)):null;
  ...
  const saved=await write(edit.type,p);if(saved&&shareAfterSave){const created=data.events.find(x=>!beforeEventIds.has(x.id));if(created)openShare(created);}
 });
 document.addEventListener('click',async e=>{
  ...
  try{
   ...
   if(a==='share-event'){const e=data.events.find(x=>x.id===id);if(e)openShare(e);return;}
   if(a==='copy-event'){const e=data.events.find(x=>x.id===id);if(e){try{await navigator.clipboard.writeText(M.shareEvent(e,data.team,location.origin,location.pathname).url);toast('イベントのリンクをコピーしました');}catch{toast('コピーできませんでした。共有内容のURLを選択してコピーしてください',true);}}return;}
   ...
   if(a==='tab'){if(b.dataset.tab==='notices'||(b.dataset.tab==='ledger'&&!admin))return;tab=b.dataset.tab;eventId='';render();return;}
   if(a.startsWith('calendar-')){if(a==='calendar-today')calendarMonth=M.today().slice(0,7);else{const [y,m]=calendarMonth.split('-').map(Number);calendarMonth=new Date(Date.UTC(y,m-1+(a==='calendar-next'?1:-1),1)).toISOString().slice(0,7);}render();return;}
   if(a==='past'){past=!past;render();return;}
   if(a==='open-event'){eventId=id;render();window.scrollTo({top:0,left:0,behavior:'instant'});return;}
   ...
   if(a==='grant-admin'){
    if(!admin)return;const m=data.members.find(x=>x.id===id);if(!m||!m.active||m.squad!=='main'||!m.email||m.is_admin)return;
    if(confirm(m.name+'さんに管理者権限を付与しますか？\n会計・車のナンバーの閲覧、全員の出欠・予定・名簿の編集、他のメンバーへの管理者権限付与が可能になります。'))await write('grant_admin',{id:m.id});return;
   }
   if(a==='member-new')openForm('member');
   if(a==='member-edit')openForm('member',data.members.find(m=>m.id===id));
   if(a==='member-number-sort'){memberNumberDesc=!memberNumberDesc;render();return;}
   if(a==='archive'&&confirm(b.dataset.active==='true'?'このメンバーを名簿に戻しますか？':'このメンバーを削除しますか？ログインと今後の出欠対象から外れます。過去の記録は保持され、管理者は元に戻せます。'))await write('archive_member',{id,active:b.dataset.active==='true'});
   if(a==='delete-member'){if(!admin)return;const m=data.members.find(x=>x.id===id);if(!m||m.active)return;const entered=prompt('完全削除すると過去の出欠も消え、元に戻せません。続けるにはメンバー名「'+m.name+'」を入力してください。','');if(entered===m.name)await write('delete_member',{id:m.id});else if(entered!==null)toast('名前が一致しないため削除しませんでした',true);}
   if(a==='event-new')openForm('event');
   if(a==='event-edit'||a==='event-copy')openForm('event',data.events.find(x=>x.id===id),{copy:a==='event-copy'});
   if(a==='answer'){const ev=data.events.find(x=>x.id===b.dataset.ev);const days=ev?Math.ceil((new Date(ev.event_date+'T00:00:00+09:00')-new Date(M.today()+'T00:00:00+09:00'))/86400000):99;if(days>=0&&days<=14&&!confirm('開催日まで2週間以内です。変更内容をチームLINEにもコメントしてください。出欠入力を続けますか？'))return;openForm('answer',answer(b.dataset.ev,b.dataset.mid)||{},{ev:b.dataset.ev,mid:b.dataset.mid});}
   ...
   if(a==='export-event'){if(!admin)return;await load();const ev=data.events.find(x=>x.id===id);if(!ev)throw new Error('予定が見つかりません');const cars=M.count(data,ev).cars;if(cars>=16&&!confirm('車が'+cars+'台います（16台以上）。ファイルを出力しますか？'))return;const file=AttendanceExport.create(data,ev);if(file.warnings.length&&!confirm('車・バイクの予定やナンバーに未入力があります。未入力の旨を記載して出力しますか？\n'+file.warnings.join('\n')))return;AttendanceExport.download(file);toast(file.count+'名のExcelを出力しました');return;}
 ...
 document.addEventListener('keydown',e=>{if((e.key==='Enter'||e.key===' ')&&e.target.matches('.schedule-event')){e.preventDefault();e.target.click();}});
