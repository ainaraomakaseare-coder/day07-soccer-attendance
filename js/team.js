...
(() => {
 ...
 function eventsView(sel){
  ...
  return '<div class="section-head"><div><h2>イベント一覧</h2><p class="hint">予定の行を押すと、その日の参加状況と出欠登録を開きます。</p></div>'+ (admin?btn('＋ 予定を作る','event-new','',''):'')+'</div><div class="toolbar">'+btn(past?'これからの予定':'過去の予定','past')+(admin?btn('＋ メンバー追加','member-new'):'')+'</div>'+months.map(month=>'<section class="panel schedule-month"><h3>'+esc(month.replace('-','年'))+'月</h3><div class="table-wrap"><table><thead><tr><th>日程</th><th>時間</th><th>場所</th><th>自分の出欠</th><th>出席</th></tr></thead><tbody>'+list.filter(e=>e.event_date.startsWith(month)).map(e=>{const a=sel?answer(e.id,sel.id):null,c=M.count(data,e);return '<tr class="schedule-event" role="button" tabindex="0" data-action="open-event" '+attr('id',e.id)+' aria-label="'+esc(dateLabel(e.event_date)+' '+(e.place||'場所未定')+'の出欠を開く')+'"><td><strong class="schedule-date">'+esc(dateLabel(e.event_date))+'</strong></td><td>'+esc(e.start_time?.slice(0,5)||'時間未定')+(e.end_time?' – '+esc(e.end_time.slice(0,5)):'')+'</td><td>'+esc(e.place||'場所未定')+'<div class="hint">'+(e.kind==='match'?'試合':'練習')+'</div></td><td>'+(a?.status==='yes'?'○ 出席':a?.status==='no'?'× 欠席':'未回答')+'</td><td>'+c.total+'人</td></tr>';}).join('')+'</tbody></table></div></section>').join('')+(list.length?'':empty('予定がまだありません。'));
 }
