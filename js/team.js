...
(() => {
 ...
 function gate(message=''){
  app.innerHTML=`<section class="panel gate"><span class="eyebrow">TEAM NOTE</span><h1>自分の出欠を、<br>すぐ回答。</h1><p>メインメンバーはGoogleでログインしてください。登録メールから自動で紐づくので、名前を選ぶ操作はありません。</p>${message?`<p class="inline-error">${esc(message)}</p>`:''}${btn('Googleでログイン','signin','','')}${Auth.loggedIn()?btn('ログアウト','logout'):''}</section>`;
 }

 function render(){
  if(!data)return;
  const sel=members().find(m=>m.id===selected);
  const nav=[['events','出欠'],['members','名簿'],['notices','伝達事項'],['annual','参加回数'],['ledger','会計'],['history','変更履歴'],...(admin?[['settings','設定']]:[])];
  app.innerHTML=`<section class="heading"><div><span class="eyebrow">${admin?'MANAGER DESK':'TEAM NOTE'}</span><h1>${esc(data.team)} <span class="tag">${admin?'管理者':data.me?'メイン':'ジュニア・共同編集'}</span></h1><div class="muted">出欠も、チームのことも。ここから。</div></div><div class="actor">${data.me?`<strong>${esc(data.me.name)}さん</strong>`:`<label for="actor">入力者（履歴用）</label><input id="actor" maxlength="80" placeholder="管理担当者名" value="${esc(actor)}">`}${btn('更新','refresh')}${btn('閉じる','logout')}</div></section>
 ...
 function eventsView(sel){
  ...
  const list=data.events.filter(e=>e.squad===squad&&(past?e.event_date<M.today():e.event_date>=M.today()));if(past)list.reverse();
  return `<div class="section-head"><div><h2>${past?'これまでの予定':'これからの予定'}</h2><div class="hint">${squad==='junior'?'名簿の各行から出欠を変更できます。':admin?'予定を管理し、「誰が来る？」から代理回答できます。':'ログインした自分の出欠を回答できます。'}</div></div>${admin?btn('＋ 予定を作る','event-new','',''):''}</div>
  <div class="toolbar">${squad==='junior'||admin?btn('＋ メンバー追加','member-new'):''}${btn(past?'これからの予定':'過去の予定','past')}</div><p class="hint">締切後も変更可能です。${data.me?'他のメンバーの出欠は「誰が来る？」で確認できます。':''}</p>
  <div class="cards">${list.map(e=>card(e,sel)+(squad==='junior'?juniorRows(e):'')).join('')}</div>${list.length?'':empty('予定がまだありません。管理者が予定を追加するとここに表示されます。')}`;
 }
 function card(e,sel){const c=M.count(data,e);const a=sel?answer(e.id,sel.id):null;const overdue=e.deadline&&e.deadline<M.today();
  return `<article class="card"><div class="event-top"><div class="date-tile"><span>${Number(e.event_date.slice(5,7))}月</span><strong>${Number(e.event_date.slice(8))}</strong></div><div><span class="tag">${e.kind==='match'?'試合':'練習'}</span>${e.cleaning?' <span class="tag warn">掃除の日</span>':''}<div class="event-title">${esc(e.place||'場所未定')}</div><div class="muted">${esc(dateLabel(e.event_date))} · ${esc(e.start_time?.slice(0,5)||'時間未定')}${e.end_time?' – '+esc(e.end_time.slice(0,5)):''}</div></div></div>
  <div class="detail">${e.deadline?`回答期限 ${esc(dateLabel(e.deadline))} ${overdue?'<span class="tag warn">期限を過ぎています</span>':''}<br>`:''}${e.booking_person?'予約担当 '+esc(e.booking_person)+'<br>':''}${esc(e.note)}</div>
  ${sel?`<div class="quick-label">${esc(sel.name)}さんの出欠</div><div class="quick-buttons">${btn('○ 出席','quick',`${attr('ev',e.id)} data-status="yes"`,a?.status==='yes'?'chosen yes-btn':'secondary')}${btn('× 欠席','quick',`${attr('ev',e.id)} data-status="no"`,a?.status==='no'?'chosen no-btn':'secondary')}</div>${a?.status==='yes'&&e.asks_car?`<div class="car-quick">車の利用予定は？<div class="actions">${btn('利用する','quick-car',`${attr('ev',e.id)} data-car="yes"`,a.car==='yes'?'small':'secondary small')}${btn('利用しない','quick-car',`${attr('ev',e.id)} data-car="no"`,a.car==='no'?'small':'secondary small')}</div>${a.car==='yes'?`<p class="hint">ナンバー：${esc(a.vehicle_plate)} ${btn('変更','answer',`${attr('ev',e.id)} ${attr('mid',sel.id)}`)}</p>`:''}</div>`:''}<div class="actions">${btn('未回答に戻す','quick',`${attr('ev',e.id)} data-status=""`)}${btn('遅刻などを連絡','answer',`${attr('ev',e.id)} ${attr('mid',sel.id)}`)}</div>`:''}
  <div class="metrics"><div><b>${c.total}</b>出席（助っ人${c.guests}）</div><div><b>${c.no}</b>欠席</div><div><b>${c.unknown}</b>未回答</div><div><b>${e.asks_car?c.cars:'—'}</b>車</div></div>
  ${squad==='junior'?`<div class="stats-role">スタッフ ${c.staff} / AC ${c.ac} / 選手 ${c.player} / 体験 ${c.memberGuest}</div>`:''}
  <div class="actions">${btn('誰が来る？','open-event',attr('id',e.id))}${admin?btn('編集','event-edit',attr('id',e.id))+btn('複製','event-copy',attr('id',e.id)):''}</div></article>`;
 }
 ...
 function syncPlate(){const f=document.getElementById('edit-form'),plate=f.elements.vehicle_plate;if(!plate)return;const car=f.elements.car;const needs=f.elements.status?.value==='yes'&&(car?.type==='checkbox'?car.checked:car?.value==='yes');plate.required=needs;plate.disabled=!needs;plate.closest('label').hidden=!needs;}
 function eventView(e){const gs=data.guests.filter(g=>g.event_id===e.id);return `<div class="section-head"><div><h2>${esc(dateLabel(e.event_date))} ${esc(e.place)}</h2><div class="muted">メンバー ${members().length}人 · 助っ人 ${gs.filter(g=>g.status==='yes').length}人出席</div></div>${btn('予定へ戻る','event-back')}</div>
  <div class="table-wrap"><table><thead><tr><th>名前</th><th>区分</th><th>出欠</th><th>車</th><th>伝達・確認</th><th>操作</th></tr></thead><tbody>${members().map(m=>{const a=answer(e.id,m.id);return `<tr><td>${esc(m.name)}</td><td>${roleName[m.member_role]}</td><td class="${a?.status||'unknown'}">${a?.status==='yes'?'○ 出席':a?.status==='no'?'× 欠席':'未回答'}</td><td>${a?.car==='yes'?'利用する':a?.car==='no'?'利用しない':'—'}${a?.vehicle_plate?`<div class="hint">${esc(a.vehicle_plate)}</div>`:''}</td><td class="note">${esc(a?.note)}${a?.confirmed?' ✓確認済み':''}</td><td>${admin||squad==='junior'||data.me?.id===m.id?btn('回答・修正','answer',`${attr('ev',e.id)} ${attr('mid',m.id)}`):'—'}</td></tr>`;}).join('')}</tbody></table></div>
  <div class="section-head"><h3>この日の助っ人</h3>${btn('＋ ゲストを追加','guest-new',attr('ev',e.id),'')}</div>${gs.length?`<div class="table-wrap"><table><thead><tr><th>名前</th><th>声掛け担当</th><th>出欠</th><th>車</th><th>伝達事項</th><th></th></tr></thead><tbody>${gs.map(g=>`<tr><td>${esc(g.name)}</td><td>${esc(g.invited_by)}</td><td>${g.status==='yes'?'○ 出席':'× 欠席'}</td><td>${g.car?'利用する':'利用しない'}${g.vehicle_plate?`<div class="hint">${esc(g.vehicle_plate)}</div>`:''}</td><td class="note">${esc(g.note)}</td><td>${admin||squad==='junior'||g.created_by===data.me?.id?btn('修正','guest-edit',attr('id',g.id)):'—'}</td></tr>`).join('')}</tbody></table></div>`:empty('助っ人はまだ登録されていません。')}${admin?`<p>${btn('この予定を削除','delete-event',attr('id',e.id),'danger small')}</p>`:''}`;}
 ...
 function settingsView(){return `<div class="settings"><h2>チーム設定</h2><div class="panel"><p>年度起算月：${data.year_start_month}月</p><p>参加費の初期値：${money(data.main_fee)}</p><p class="hint">年度起算月・開始残高を変更すると集計が変わります。</p>${btn('設定を変更','settings-edit','','')}</div><h3>メンバーに案内する</h3><div class="panel"><p>このURLをメンバーに共有してください。登録したGoogleメールでログインすると、自分の出欠が表示されます。</p><a href="./team.html">メンバー画面を開く</a>${btn('メンバー用URLをコピー','copy-main-link')}</div></div>`;}
 ...
 function openForm(type,v={},extra={}){
  ...
  if(type==='answer'){title=memberName(extra.mid)+'さんの回答';const e=data.events.find(e=>e.id===extra.ev);html=select('status','出欠',v.status||'',[['','未回答'],['yes','○ 出席'],['no','× 欠席']])+(e.asks_car?select('car','車の利用予定は？',v.car||'',[['','未回答'],['yes','利用する'],['no','利用しない']])+field('vehicle_plate','ナンバー全体（例：横浜 300 あ 1234）',v.vehicle_plate||((extra.mid===data.me?.id?data.me:data.members.find(m=>m.id===extra.mid))?.vehicle_plate)||'','text',{maxlength:40,wide:true}):'')+area('note','遅刻など、この日の伝達事項',v.note)+check('confirmed','伝達を確認した',v.confirmed)+'<p class="hint wide">「未回答」は回答とこの日の伝達を取り消します。一般の連絡は伝達事項タブに残せます。</p>';}
  if(type==='guest'){title=v.id?'助っ人を修正':'この日の助っ人を追加';html=field('name','助っ人の名前',v.name,'text',{required:true,maxlength:80})+(data.me?`<p>声掛け担当：${esc(data.me.name)}</p>`:field('invited_by','声掛け担当',v.invited_by||actor))+select('status','出欠',v.status||'yes',[['yes','○ 出席'],['no','× 欠席']])+(data.events.find(e=>e.id===(extra.ev||v.event_id))?.asks_car?check('car','車の利用予定は？ 利用する',v.car)+field('vehicle_plate','ナンバー全体（例：横浜 300 あ 1234）',v.vehicle_plate,'text',{maxlength:40,wide:true}):'')+area('note','伝達事項',v.note);edit.ev=extra.ev||v.event_id;}
 ...
 async function load(){data=await DB.rpc('team_home',{p_key:key,p_admin:admin},!admin&&!key);squad='main';selected=data.me?.id||'';if(data.me)actor=data.me.name;render();}
 async function write(action,payload){
  ...
  try{data=await DB.rpc('team_write',{p_key:key,p_admin:admin,p_version:data.version,p_actor:actor,p_action:action,p_data:payload},!admin&&!key);selected=data.me?.id||'';dialog.close();render();toast('保存しました');return true;}
 ...
 document.addEventListener('change',e=>{
  ...
  if(e.target.closest('#edit-form'))syncPlate();
 ...
 document.addEventListener('submit',async e=>{
  ...
  if(edit.type==='answer'){p.member_id=edit.mid;p.event_id=edit.ev;}
 ...
 document.addEventListener('click',async e=>{
  ...
  try{
   ...
   if(a==='logout'){if(!admin&&!key)await Auth.signOut();location.href='./team.html';return;}
   if(a==='copy-main-link'){await navigator.clipboard.writeText(location.origin+location.pathname);toast('メンバー用URLをコピーしました');return;}
   if(a==='tab'){tab=b.dataset.tab;eventId='';render();return;}
   ...
   if(a==='quick'||a==='quick-car'){const mid=data.me?.id||b.dataset.mid||selected,old=answer(b.dataset.ev,mid)||{};
    if(a==='quick-car'&&b.dataset.car==='yes'){openForm('answer',{...old,status:'yes',car:'yes'},{ev:b.dataset.ev,mid});return;}
    await write('answer',{member_id:mid,event_id:b.dataset.ev,status:a==='quick'?b.dataset.status:old.status||'yes',car:a==='quick-car'?b.dataset.car:old.car||'',vehicle_plate:old.vehicle_plate||'',note:old.note||'',confirmed:old.confirmed||false});return;}
 ...
 async function start(){
  ...
  admin=params.has('admin');key=params.get(admin?'admin':'team')||'';
  ...
  if((key&&!/^[a-f0-9]{32}$/.test(key))||(!key&&!admin&&!Auth.loggedIn())){gate();return;}
