...
(() => {
 ...
 function card(e,sel){const c=M.count(data,e);const a=sel?answer(e.id,sel.id):null;const overdue=e.deadline&&e.deadline<M.today();
  ...
  ${sel?`<div class="quick-label">${esc(sel.name)}さんの出欠</div><div class="quick-buttons">${btn('○ 出席','quick',`${attr('ev',e.id)} data-status="yes"`,a?.status==='yes'?'chosen yes-btn':'secondary')}${btn('× 欠席','quick',`${attr('ev',e.id)} data-status="no"`,a?.status==='no'?'chosen no-btn':'secondary')}</div>${a?.status==='yes'&&e.asks_car?`<div class="car-quick">車の利用予定は？<div class="actions">${btn('利用する','quick-car',`${attr('ev',e.id)} data-car="yes"`,a.car==='yes'?'small':'secondary small')}${btn('利用しない','quick-car',`${attr('ev',e.id)} data-car="no"`,a.car==='no'&&!a.uses_bicycle?'small':'secondary small')}${btn('自転車','quick-car',`${attr('ev',e.id)} data-car="bicycle"`,a.uses_bicycle?'small':'secondary small')}</div>${a.car==='yes'?`<p class="hint">ナンバー：${esc(a.vehicle_plate)} ${btn('変更','answer',`${attr('ev',e.id)} ${attr('mid',sel.id)}`)}</p>`:''}</div>`:''}<div class="actions">${btn('未回答に戻す','quick',`${attr('ev',e.id)} data-status=""`)}${btn('遅刻などを連絡','answer',`${attr('ev',e.id)} ${attr('mid',sel.id)}`)}</div>`:''}
 ...
 function eventView(e){const gs=data.guests.filter(g=>g.event_id===e.id);return `<div class="section-head"><div><h2>${esc(dateLabel(e.event_date))} ${esc(e.place)}</h2><div class="muted">メンバー ${members().length}人 · 助っ人 ${gs.filter(g=>g.status==='yes').length}人出席</div></div><div class="actions">${admin?btn('Excelを出力','export-event',attr('id',e.id),''):''}${btn('予定へ戻る','event-back')}</div></div>
