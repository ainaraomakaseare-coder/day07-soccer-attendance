...
(() => {
 ...
 let data=null,key='',admin=false,squad='main',tab='events',eventId=new URLSearchParams(location.search).get('event')||'',past=false,selected='',actor=get('actor')||'管理者',busy=false,edit=null;
 let year=M.yearFor(M.today(),4),calendarMonth=M.today().slice(0,7),timer;
 const roleName={player:'選手',staff:'スタッフ',ac:'AC',guest:'体験・ゲスト'};
 const actionName={grant_admin:'管理者権限を付与',reorder_members:'名簿の順番を変更',answer:'出欠を更新',member:'名簿を更新',archive_member:'メンバーの利用状態を変更',delete_member:'メンバーを完全削除',event:'予定を保存',delete_event:'予定を削除',guest:'助っ人を更新',notice:'伝達事項を更新',ledger:'会計を保存',delete_ledger:'会計を削除',bib:'ビブスを更新',settings:'設定を更新',rotate_link:'共有リンクを変更',undo_answer:'出欠を取り消し'};
 ...
 function membersView(){const list=data.members.filter(m=>m.squad===squad&&(m.active||admin));return `<div class="section-head"><div><h2>メンバー名簿</h2><div class="hint">${squad==='junior'?'名前と区分だけで追加できます。':'Googleの登録メールと自動で紐づきます。名簿の追加・編集は管理者が行います。'}${admin?' ▲▼で表示順を変更できます。':''}</div></div>${admin||squad==='junior'?btn('＋ 追加','member-new','',''):''}</div><div class="table-wrap"><table><thead><tr><th>順番</th><th>番号</th><th>名前</th><th>区分</th><th>学年</th><th>所属・Jr.情報</th><th>代理入力者</th><th>背番号</th><th></th></tr></thead><tbody>${list.map((m,i)=>`<tr><td>${admin?btn('▲','member-move',`${attr('id',m.id)} data-direction="up"${i?'':' disabled'}`)+btn('▼','member-move',`${attr('id',m.id)} data-direction="down"${i<list.length-1?'':' disabled'}`):i+1}</td><td>${esc(m.number)}</td><td>${esc(m.name)}${m.active?'':'（削除済み）'}</td><td>${roleName[m.member_role]}</td><td>${esc(m.grade)}</td><td class="note">${esc(m.affiliation)}</td><td>${esc(m.proxy_name)}</td><td>${esc(m.jersey)}</td><td>${m.active&&(admin||squad==='junior')?btn('編集','member-edit',attr('id',m.id)):''}${admin&&m.squad==='main'?(m.is_admin?'<span class="tag">管理者</span>':m.active&&m.email?btn('管理者権限を付与','grant-admin',attr('id',m.id)):''):''}${admin?btn(m.active?'メンバーを削除':'元に戻す','archive',`${attr('id',m.id)} data-active="${!m.active}"`,m.active?'danger small':'secondary small'):''}${admin&&!m.active?btn('完全に削除','delete-member',attr('id',m.id),'danger small'):''}</td></tr>`).join('')}</tbody></table></div>${members().length?'':empty('最初のメンバーを追加しましょう。')}`;}
 ...
 document.addEventListener('click',async e=>{
  ...
  try{
   ...
   if(a==='member-move'){if(!admin||b.disabled)return;const list=data.members.filter(m=>m.squad===squad),from=list.findIndex(m=>m.id===id),to=from+(b.dataset.direction==='up'?-1:1);if(from<0||to<0||to>=list.length)return;[list[from],list[to]]=[list[to],list[from]];await write('reorder_members',{squad,ids:list.map(m=>m.id)});}
